...
var THEMES = [
  { icon: "\u2600\uFE0F", title: "Solar energy & climate change", text: "How a warming climate reshapes Africa's solar and photovoltaic potential.", keys: ["solar", "photovoltaic", "pv ", "energy", "power"] },
  { icon: "\uD83E\uDDE0", title: "Physics-constrained machine learning", text: "Deep learning that respects atmospheric physics to correct climate-model bias.", keys: ["physics", "deep learning", "machine learning", "bias correction", "neural"] },
  { icon: "\uD83C\uDF00", title: "Tropical cyclones", text: "Dynamics and intensity of tropical cyclones in a changing climate.", keys: ["cyclone", "typhoon", "hurricane"] },
  { icon: "\uD83D\uDD25", title: "Heat, drought & compound extremes", text: "Heatwaves, drought persistence and co-occurring hot-dry extremes.", keys: ["heat", "drought", "compound", "precipitation", "rainfall", "extreme"] }
];

var NEWS = [
  { year: "2026", title: "PhD conferred", text: "Completed my PhD in Engineering Mechanics and Energy at the University of Tsukuba." },
  { year: "2026", title: "New paper in npj Clean Energy", text: "Climate-driven synchronization of solar extremes across Africa's power pools.", href: NPJ },
  { year: "2026", title: "New paper in Communications Earth & Environment", text: "Latest work on African climate extremes." },
  { year: "2026", title: "New paper in Sustainable Energy Technologies and Assessments", text: "Extreme weather and cascading photovoltaic power vulnerabilities.", href: "https://doi.org/10.1016/j.seta.2025.104776" },
  { year: "2025", title: "PhysConDL published in Renewable Energy", text: "Physics-constrained deep learning bias correction of CMIP6 solar radiation.", href: PHYS }
];

var METHODS = ["Physics-constrained deep learning", "CMIP6 bias correction", "Climate extremes", "Solar & wind meteorology", "Tropical cyclone dynamics", "Drought & compound events"];
...
function useReveal() {
  ...
  useEffect(function () {
    ...
    var io = new IntersectionObserver(function (es) { if (es[0].isIntersecting) { setShown(true); io.disconnect(); } }, { threshold: 0.15 });
    io.observe(el);
...
function PowerPools() {
  ...
  return (
    <Section id="power-pools" dark kicker="Interactive \u00b7 npj Clean Energy 2026" title="When the sun dims across a continent" intro="Africa's five regional power pools are meant to back each other up. My research shows low-sunshine extremes increasingly strike them at the same time. Select a pool to explore.">
      <div className="rx-pp">
        ...
        <div className="rx-pp-panel">
          ...
          <p className="rx-pp-desc">When extremes here coincide with those in other pools, neighbours have less spare solar power to cover the shortfall.</p>
          ...
          <a className="rx-cta" href={NPJ} target="_blank" rel="noopener noreferrer">Read the study <span>&rarr;</span></a>
        </div>
      </div>
    </Section>
  );
    ...
    <Section id="method" kicker="Method" title="PhysConDL in three steps" intro="Machine learning is powerful, but on its own it can break the laws of physics. PhysConDL keeps it honest.">
      <div className="rx-method">
        ...
        <div className="rx-stat">
          ...
          <a className="rx-cta dark" href={PHYS} target="_blank" rel="noopener noreferrer">Renewable Energy, 2025 <span>&rarr;</span></a>
        </div>
      </div>
    </Section>
  );
...
function Themes({ pubs }) {
  return (
    <Section id="themes" soft kicker="Research themes" title="What I work on">
      <div className="rx-themes">
        {THEMES.map(function (th) {
          var hits = pubs.filter(function (p) {
            var t = " " + (p.title || "").toLowerCase() + " ";
            return th.keys.some(function (k) { return t.indexOf(k) !== -1; });
          }).sort(function (a, b) { return (b.year || 0) - (a.year || 0); });
          return (
            <div key={th.title} className="rx-theme">
              <div className="rx-theme-top"><span className="rx-theme-i">{th.icon}</span>{hits.length > 0 && <span className="rx-count">{hits.length} {hits.length === 1 ? "paper" : "papers"}</span>}</div>
              <h3>{th.title}</h3>
              <p>{th.text}</p>
              <ul>
...
function News() {
  return (
    <Section id="news" kicker="News" title="Recent milestones">
      <ul className="rx-news">
        {NEWS.map(function (n, i) {
          var body = <><h3>{n.title}</h3><p>{n.text}</p></>;
          return (
            <li key={i} className="rx-news-i">
              <span className="rx-news-y">{n.year}</span>
              <div className="rx-news-b">{n.href ? <a href={n.href} target="_blank" rel="noopener noreferrer">{body}</a> : body}</div>
            </li>
          );
        })}
      </ul>
  ...
  return (
    <Section id="collaborate" kicker="Collaboration" title="Let's work together">
      <div className="rx-collab">
        <div>
          <p className="rx-intro">I'm keen to collaborate with climate modellers, energy researchers and data scientists. Areas I bring:</p>
...
.rx-method{display:grid;grid-template-columns:1.4fr 1fr;gap:40px;align-items:center}
...
.rx-stat{background:${navy};color:#fff;border-radius:18px;padding:34px 30px;box-shadow:0 24px 50px rgba(12,30,60,.25)}
.rx-stat-v{font-family:'Cormorant Garamond',serif;font-size:62px;font-weight:700;color:${goldL};line-height:1}
.rx-stat-l{font:500 15px 'DM Sans',sans-serif;color:rgba(255,255,255,.75);margin:8px 0 22px}
...
.rx-stat .rx-cta.dark{color:${goldL}}
.rx-themes{display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:20px;margin-top:10px}
...
.rx-news{list-style:none;margin:10px 0 0;padding:0 0 0 18px;border-left:2px solid #eadfbf;max-width:760px}
.rx-news-i{position:relative;display:flex;gap:18px;padding:0 0 26px 18px}
.rx-news-i:before{content:"";position:absolute;left:-26px;top:4px;width:12px;height:12px;border-radius:50%;background:${gold};box-shadow:0 0 0 4px #fff,0 0 0 5px #eadfbf}
.rx-news-y{flex:none;font:700 12px 'DM Sans',sans-serif;color:${navy};background:#f3ead0;border-radius:6px;padding:3px 8px;height:fit-content}
.rx-news-b h3{font:700 16px 'DM Sans',sans-serif;color:${navy};margin:0 0 3px}
.rx-news-b p{font:400 14px/1.5 'DM Sans',sans-serif;color:#5b6475;margin:0}
.rx-news-b a{text-decoration:none}
.rx-news-b a:hover h3{color:${gold}}
...
.rx-btn{text-align:center;font:700 14px 'DM Sans',sans-serif;padding:13px 20px;border-radius:10px;text-decoration:none;color:${navy};border:1.5px solid ${navy};transition:all .2s}
...
export default function ResearchExtras({ C, img, pubs }) {
  return (
    <div className="rx">
      <style>{css(C)}</style>
      <PowerPools />
